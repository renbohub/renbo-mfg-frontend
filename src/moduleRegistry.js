const modules = [
  ...
  },
  {
    ...
    pages: [
      { ...dashboardPage("Planning Performance", "MPP plan dibanding realisasi produksi aktual per bulan."), navHidden: true },
      page("demand-planning", "Demand Planning", "Forecast delivery target dengan Actual SO dan rolling capacity", "/api/planning/demand-planning", "deliveryTargetId", [col("sourceNumber", "Forecast"), col("customerCode", "Customer"), col("partCode", "Part"), col("targetDeliveryDate", "Target Delivery", "date"), col("forecastQty", "Forecast", "number"), col("actualSalesOrderQty", "Actual SO", "number"), col("demandQty", "Effective Demand", "number"), col("finalPriorityScore", "Priority", "number"), col("feasibilityStatus", "Feasibility", "status"), col("planningStatus", "Planning Status", "status")]),
      page("master-production-schedule", "Rolling Master Production Schedule", "Revision MPS dengan demand horizon; bulan hanya filter delivery", "/api/planning/mps", "mpsNumber", [col("mpsNumber", "MPS ID"), col("periodStart", "Horizon Mulai", "date"), col("periodEnd", "Horizon Selesai", "date"), col("mpsName", "Nama MPS"), col("totalPlannedQty", "Qty Plan", "number"), col("lifecycleStatus", "Lifecycle", "status"), col("replanRequired", "Replan", "boolean"), col("status", "Status", "status")]),
      { ...page("mps-recovery", "MPS Recovery", "Kanban tindak lanjut recovery MPS lintas departemen", "/api/planning/demand-planning/recovery-plans", "id", [col("recovery", "Recovery"), col("feedbackStatus", "Feedback Status", "status"), col("dept", "Dept")]), icon: "grid" },
      page("material-requirements-planning", "Material Requirements Planning", "Monthly MRP versioned dengan exact-date netting dan customer pegging", "/api/planning/mrp", "runNumber", [col("planNumber", "MRP Plan"), col("runNumber", "Run / Revision"), col("planRevision", "Rev", "number"), col("isCurrentPlan", "Current", "boolean"), col("mpsNumber", "MPS"), col("planningMonth", "Planning Month", "date"), col("totalRequirements", "Requirements", "number"), col("totalPlannedOrders", "Planned Order", "number"), col("status", "Status", "status")]),
      page("monthly-production-plans", "Production Plans", "Rencana pemenuhan produksi berdasarkan demand-phase horizon lintas bulan", "/api/planning/monthly-production-plans", "planNumber", [col("planNumber", "Plan ID"), col("periodStart", "Horizon Mulai", "date"), col("schedulingHorizonEnd", "Horizon Selesai", "date"), col("targetQty", "Target", "number"), col("actualQty", "Actual", "number"), col("status", "Status", "status")]),
      { ...page("capacity-planning", "Capacity Planning", "Heatmap capacity harian untuk mengalokasikan Production Plan horizon", "/api/planning/capacity-planning", "machineCode", [col("machineCode", "Mesin"), col("machineName", "Nama"), col("lineCode", "Line"), col("status", "Status", "status")]), navHidden: true },
      page("daily-production-plans", "Daily Production Plan", "Approved finite-capacity allocation yang dikonsumsi Production", "/api/planning/monthly-production-plans/daily-plans", "scheduleNumber", [col("scheduleNumber", "No. Daily Plan"), col("scheduleDate", "Date", "date"), col("shift", "Shift"), col("machineCode", "Machine"), col("processName", "Process"), col("partCode", "Part"), col("plannedQty", "Qty", "number"), col("customerCode", "Customer"), col("demandSourceNumber", "SO/Forecast"), col("customerTargetDate", "Customer Delivery", "date"), col("fgRequiredDate", "FG Required", "date"), col("priorityClass", "Priority", "status"), col("materialReadinessStatus", "Material", "status"), col("predecessorStatus", "Predecessor", "status"), col("vendorStatus", "Vendor", "status"), col("lateRisk", "Late Risk", "status")]),
      { ...page("control-tower", "Control Tower", "Visibilitas demand, delivery, dan risiko lintas proses", "/api/dashboard/control-tower", "soNumber", [col("soNumber", "Sales Order"), col("customer.customerName", "Customer"), col("deliveryDate", "Required", "date"), col("orderedQty", "Demand", "number"), col("deliveredQty", "Delivered", "number"), col("outstandingQty", "Outstanding", "number"), col("purchaseDemandQty", "PR Demand", "number"), col("supplierAllocatedQty", "Supplier Allocation", "number"), col("supplierAllocationStatus", "Supplier Status", "status"), col("purchaseOrderedQty", "PO Ordered", "number"), col("purchaseOrderControlStatus", "Order Status", "status"), col("risk", "Risk", "status")]), navHidden: true },
      { ...page("planned-orders", "Planned Orders", "Usulan order dari hasil perencanaan", "/api/planning/mrp/planned-orders", "orderNumber", [col("orderNumber", "No. Planned Order"), col("partCode", "Part Code"), col("part.partName", "Part Name"), col("orderType", "Type"), col("requiredDate", "Required", "date"), col("qty", "Qty", "number"), col("supplierReadiness.supplierCode", "Supplier"), col("supplierReadiness.status", "Supplier Readiness", "status"), col("status", "Status", "status")]), navHidden: true }
    ]
  },
  {
    ...
    pages: [
      ...
      { ...page("production-logs", "Production Entries", "Pencatatan aktual shop-floor dari Daily Production Schedule", "/api/production/production-logs", "logNumber", [col("logNumber", "No. Entry"), col("logDate", "Tanggal", "date"), col("shift", "Shift"), col("outputPart.partCode", "Part Code"), col("outputPart.partNumber", "Part Number"), col("outputPart.partName", "Part Name"), col("dailyProductionSchedule.productionPlan.planNumber", "Production Plan"), col("dailyProductionSchedule.scheduleNumber", "Daily Production Schedule"), col("processCode", "Proses"), col("machineCode", "Mesin"), col("operatorName", "Operator"), col("qtyProduced", "Produced", "number"), col("qtyGood", "Good", "number"), col("qtyReject", "NG", "number"), col("downtime", "Downtime (min)", "number"), col("status", "Status", "status")]), createRoute: "/modules/production/production-logs/new" },
  ...
  {
    slug: "qc", label: "Quality Control", shortLabel: "QC", icon: "check-circle",
    description: "Judgment barang NG, release stock hasil inspeksi, serta nilai recovery rework dan scrap.", color: "blue",
    pages: [
      { ...page("ng-dispositions", "QC Rework Station", "Putuskan Qty NG menjadi rework, scrap, atau kombinasi sebelum stock bergerak", "/api/quality/ng-dispositions", "id", [col("logDate", "Tanggal", "date"), col("logNumber", "Production Entry"), col("phaseNumber", "Phase", "number"), col("partCode", "Part Code"), col("partNumber", "Part Number"), col("processName", "Proses"), col("productionLotNumber", "Production Lot"), col("reason", "Reason NG"), col("subReason", "Sub Reason"), col("qtyNg", "Qty NG", "number"), col("qtyRework", "Rework", "number"), col("qtyReject", "Scrap", "number"), col("status", "QC Status", "status")]), ngDispositionFlow: true },
      { ...page("quality-inspections", "QC Release Stock", "Periksa hasil OK dari Production Entry lalu release WIP/FG dari QC Hold", "/api/quality/quality-inspections", "inspectionNumber", [col("inspectionNumber", "No. QC"), col("inspectionDate", "Tanggal", "date"), col("productionLog.logNumber", "Production Entry"), col("productionLog.operatorName", "Operator"), col("part.partCode", "Part Code"), col("part.partNumber", "Part Number"), col("batchNumber", "Batch / Lot"), col("qtyInspected", "Qty Check", "number"), col("qtyPassed", "Qty OK", "number"), col("decision", "Decision", "status"), col("status", "Release Status", "status")]), createRoute: "/modules/qc/quality-inspections/new" },
      { ...report("qc-report", "QC Report", "Qty dan nilai Rupiah barang yang berhasil dirework serta scrap berdasarkan harga per KG", "/api/quality/reports/value"), reportMode: "quality-value", reportColumns: [
        col("qcDate", "Tanggal QC", "date"), col("logNumber", "Production Entry"), col("partCode", "Part Code"), col("partNumber", "Part No"), col("partName", "Part Name"),
        col("processCode", "Proses"), col("ngReason", "Alasan NG"), col("qtyNg", "Qty NG", "number"), col("qtyReworkDecision", "Diputuskan Rework", "number"),
        col("reworkSuccessfulQty", "Rework Berhasil", "number"), col("reworkRecoveryPercent", "Recovery %", "number"), col("unitCost", "Biaya / Unit", "currency"),
        col("reworkRecoveredValue", "Nilai Rework", "currency"), col("scrapQty", "Scrap PCS", "number"), col("unitWeightKg", "KG / PCS", "number"), col("scrapKg", "Scrap KG", "number"),
        col("scrapPricePerKg", "Harga / KG", "currency"), col("scrapValue", "Nilai Scrap", "currency"), col("valueStatus", "Kelengkapan Nilai", "status")
      ] }
    ]
  },
  {
  ...
  },
  {
    ...
    pages: [
      ...
      { ...page("stock-reservations", "Stock Reservations", "Reserve manual material ke part number sebelum MRP dijalankan", "/api/inventory/stock-reservations", "reservationNumber", [col("reservationNumber", "Reservation"), col("reservationDate", "Tanggal", "date"), col("materialCode", "Material"), col("materialName", "Material Name"), col("targetPartCode", "Untuk Part Code"), col("targetPartNumber", "Untuk Part Number"), col("targetPartName", "Part Name"), col("warehouseCode", "Warehouse"), col("rackCode", "Rack"), col("lotNumber", "Lot"), col("qtyReserved", "Reserved", "number"), col("qtyReleased", "Released", "number"), col("qtyOpen", "Open", "number"), col("uomCode", "UOM"), col("status", "Status", "status")]), createRoute: "/modules/inventory/stock-reservations/new" },
      ...
      { ...report("inventory-report", "Inventory Report", "Pilih FG untuk melihat resume child part, WIP, material, dan purchase part sesuai BOM aktif", "/api/reports/inventory"), reportMode: "inventory-traceability", reportColumns: [
        ...
        col("stockType", "Stock Type"), col("qtyOnHand", "On Hand", "number"),
  ...
  },
  {
    ...
    pages: [
      ...
      page("partner-administration", "Portal Supplier & Vendor", "Pendaftaran kiriman supplier dan pengelolaan akun portal", "/api/incoming/partner-admin/notices", "noticeNumber", [col("noticeNumber", "No. Pendaftaran"), col("poNumber", "PO"), col("status", "Status", "status")]),
