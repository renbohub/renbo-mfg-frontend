(() => {
  ...
  async function initialize() {
    try {
      ...
      const schedule = normalizeSchedule(record.rules || [], shiftPayload.data || []);
      const active = schedule.flatMap((day) => day.shifts).filter((rule) => rule.isEnabled);
      ...
      document.getElementById("wh-kpi-days").textContent = number(schedule.filter((day) => day.shifts.some((rule) => rule.isEnabled)).length, 0);
