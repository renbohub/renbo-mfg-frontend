(() => {
  ...
  function render(message = "") {
    ...
    dialog.innerHTML = `<header><div><small>${esc(result ? `${result.mpsNumber} · ${t("preview")}` : t("preview"))}</small><h2 id="ppr-title">${esc(t("review"))}</h2><p>${esc(context?.item?.partCode)} · ${esc(context?.item?.partName || "")}</p></div><button type="button" data-ppr-close aria-label="${esc(t("close"))}">×</button></header>
      ...
      <details open><summary>${esc(t("process"))} · ${esc(t("lots"))}</summary>${table([t("date"), t("machine"), "Shift", "Part / " + t("process"), t("quantity"), t("lots")], result.allocations.map(row => `<tr><td>${date(row.scheduleDate)}<small>${esc(row.plannedStartTime)}–${esc(row.plannedEndTime)}</small></td><td>${esc(row.machineCode || row.vendorCode || row.vendorId)}</td><td>${esc(row.shift)}</td><td>${esc(row.partCode)} / ${esc(row.processCode)}</td><td>${qty(row.qty)} ${esc(row.uomCode)}</td><td>${result.lots.filter(lot => lot.allocations.some(a => a.allocationId === row.allocationId)).map(lot => esc(lot.lotPlanNumber)).join("<br>") || "—"}</td></tr>`))}${!result.allocations.length ? `<p>${esc(t("noSlots"))}</p>` : ""}</details>
      <details open><summary>${esc(t("material"))}</summary>${table(["Part", t("required"), t("gross"), t("net"), t("allocations")], result.materials.map(row => `<tr><td>${esc(row.partCode)}</td><td>${date(row.materialRequiredDate || row.requiredDate)}</td><td>${qty(row.grossRequirement)} ${esc(row.uomCode)}</td><td>${qty(row.netRequirement)} ${esc(row.uomCode)}</td><td>${esc(row.supplyCustomerCode || (row.materialSupplyType === "SUPPLIER_PURCHASE" ? t("supplierPurchase") : row.materialSupplyType) || "")}</td></tr>`))}</details>` : ""}</div>
