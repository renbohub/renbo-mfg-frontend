(() => {
  ...
  const definitions = {
    ...
    orders: {
      ...
      columns: ["Planned Order", "Part", "Part Number", "Type", "Order date", "Required", "Planned qty", "Released", "Release gap", "Supplier / Vendor", "MOQ / Package", "Status", "Timing"], rows: (data) => data.plannedOrders || [], status: (row) => row.timing === "LATE_RELEASE" ? "LATE_RELEASE" : row.status,
      cells: (row) => [`<a href="/modules/planning-ppic/planned-orders/${encodeURIComponent(row.orderNumber)}"><b>${esc(row.orderNumber)}</b></a><small>${esc(row.runNumber || "")}</small>`, `<b>${esc(row.partCode)}</b>`, esc(row.partNumber || "—"), chip(row.orderType, row.orderType === "Purchase" ? "vendor" : ""), day(row.orderDate), day(row.requiredDate), `<b>${num(row.qty)}</b><small>${esc(row.uomCode || "")}</small>`, num(row.qtyReleased), `<b>${num(row.releaseGap)}</b><small>${esc(row.formula)}</small>`, esc(row.supplierCode || row.vendorCode || "Belum dipilih"), `<b>${row.purchaseQtyKg ? `${num(row.purchaseQtyKg)} kg` : row.purchasePackageQty ? num(row.purchasePackageQty) : row.lotCount ? `${num(row.lotCount)} lot` : "-"}</b><small>${row.kgPerLot ? `${num(row.kgPerLot)} kg/lot` : ""}</small>`, chip(row.status, row.releaseGap > 0 ? "blocked" : "ready"), chip(row.timing, row.timing === "LATE_RELEASE" ? "late" : "ready")],
    },
    mpp: {
