(function () {
  ...
  const config = {
    ...
    mps: { title: "Master Production Schedule", subtitle: "MPS Demand Matrix: satu planning cycle memuat sampai tiga bulan Target Delivery dan hanya memiliki satu tombol Run MRP. Monthly Production Plan tetap dibuat per bulan.", url: "/modules/api/planning-ppic/master-production-schedule?start=0&length=100", primary: "Refresh dari Demand Planning", head: ["No", "Planning Cycle", "Target Delivery", "Demand Sources", "Coverage", "Qty Plan", "Due Protection", "Status", "Aksi"] },
  ...
  function renderMonthlySummary() {
    ...
    $("mps-summary-rows").innerHTML = matrixRows.map((row) => `<tr><td class="identity"><b>${esc(row.customerCode)}</b></td><td class="part"><b>${esc(row.partCode)}</b><small class="d-block text-muted">${esc(row.partName)}</small></td>${months.map((monthKey) => {
      ...
      // monthly cells. Only Monthly Production Plan remains month-specific.
      const workflowAction = ["Confirmed", "Released"].includes(document.status)
        ? `<button data-make-plan="${esc(mpsNumber)}">Monthly Production Plan</button>`
        : "";
  ...
  function render() {
    ...
    $("ppic-footer").innerHTML = tab === "mps" ? `Menampilkan <b>${displayRows.length}</b> planning cycle dari <b>${rows.length}</b> dokumen bulanan` : `Menampilkan <b>${visible.length}</b> dari <b>${rows.length}</b> data`;
