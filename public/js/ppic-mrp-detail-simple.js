(() => {
  ...
  $("mrps-customer-supply")?.addEventListener("click", () => { location.href = `/modules/purchasing/eta-monitor?tab=ppic&source=ppic&partner=customer&customer_ops=1${key ? `&run=${encodeURIComponent(key)}` : ""}`; });
  ...
  function renderBuy(rows) {
    ...
    $("mrps-body").innerHTML = displayRows.map((row) => {
      ...
      const partCode = usage.parent?.partCode || usage.output?.partCode || "-";
       ...
       return `${groupHeader}<tr><td><b>${esc(code)}</b><small>${esc(name)}${code !== row.partCode ? ` · source ${esc(row.partCode)}` : ""}</small><small>Part Number: ${esc(partNumberOf(row))}</small><small>${esc(supplyModel.supplyLabel(row))}</small></td><td class="mrps-usage-cell"><b>${esc(partNumber)}</b><small>${esc(partCode)}</small></td><td class="mrps-process-cell"><span>${esc(routeLabel(usage))}</span></td><td class="mrps-number"><b>${num(row.grossRequirement)}</b><small>${esc(uom(row))}</small></td><td class="mrps-number"><b>${num(audit.warehouseAvailable + audit.embeddedWip)}</b><small>Available ${num(audit.warehouseAvailable)} · WIP ${num(audit.embeddedWip)}</small></td><td class="mrps-number"><b>${num(audit.firmSupply)}</b><small>${esc(uom(row))}</small></td><td class="mrps-number mrps-net-buy"><b>${num(row.netRequirement)}</b><small>${supplyModel.isCustomerSupplied(row) ? "Suplai customer · tidak dibeli" : plannedOrder ? esc(plannedOrder.orderNumber) : `Buy ${num(buy)} ${esc(uom(row))}`}</small></td><td><b>${date(timeline.materialRequired)}</b><small>${esc(row.procurementWindow || "Normal")}</small></td><td><b>${date(timeline.customerDelivery)}</b><small>${esc(row.customerCode || row.planningCustomerCode || "Demand source")}</small></td><td>${badge(row._state.label,row._state.tone)}</td><td><button class="mrps-row-action" type="button" data-mrps-detail="${esc(row._id)}">Rincian</button></td></tr>`;
    }).join("");
  }
  ...
  function renderException(rows) {
    ...
    $("mrps-body").innerHTML = rows.map((row) => `<tr><td><b>${esc(row._exceptionTitle)}</b><small>${esc(row._exceptionMessage)}</small></td><td><span class="mrps-exception-type">${esc(row._exceptionType)}</span></td><td><b>${esc(row.partCode || row.orderNumber || state.doc?.runNumber || "-")}</b><small>${esc(row.part?.partName || row.part?.partNumber || "")}</small><small>Part Number: ${esc(partNumberOf(row))}</small></td><td>${date(row.materialRequiredDate || row.productionRequiredDate || row.requiredDate)}</td><td>${esc(row._exceptionType === "REVISION" ? "Production Plan diblokir" : row._exceptionType === "PURCHASE" ? "Material readiness" : "Schedule / traceability")}</td><td>${badge(row._state.label,row._state.tone)}</td><td>${row.id ? `<button class="mrps-row-action" type="button" data-mrps-detail="${esc(row._id)}">Rincian</button>` : "-"}</td></tr>`).join("");
  }
  function partNumberOf(row, code = row.partCode) {
    if (code === row.partCode && (row.partNumber || row.part?.partNumber)) return row.partNumber || row.part.partNumber;
    const source = [...(state.doc?.requirements || []), ...(state.doc?.mPlusOnePreview?.requirements || [])].find(item => item.partCode === code);
    return source?.part?.partNumber || source?.partNumber || '—';
  }
  function renderOrders(rows) {
    ...
    $("mrps-body").innerHTML = rows.map((row) => {
      ...
      return `<tr><td><b>${esc(row.orderNumber)}</b><small>${esc(row.orderType || "Purchase")}</small></td><td><b>${esc(row.partCode)}</b><small>${esc(row.partName || row.part?.partName || "-")}</small><small>Part Number: ${esc(partNumberOf(row))}</small></td><td class="mrps-number"><b>${num(row.qty)}</b><small>Sisa ${num(remaining)} ${esc(row.uomCode || "")}</small></td><td>${date(row.orderDate)}</td><td>${date(row.requiredDate)}</td><td>${request?.prNumber ? `<a href="/modules/purchasing/purchase-requisitions/${encodeURIComponent(request.prNumber)}">${esc(request.prNumber)}</a>` : "-"}</td><td>${badge(row._state.label,row._state.tone)}</td><td><button class="mrps-row-action" type="button" data-mrps-detail="${esc(row._id)}">Rincian</button></td></tr>`;
    }).join("");
  }
  function renderPegging(rows) {
    ...
    $("mrps-body").innerHTML = rows.map((row) => `<tr><td><b>${esc(row.customerCode || "-")}</b><small>${esc(row.sourceNumber || row.sourceType || "-")}</small></td><td><b>${esc(row.fgPartCode || "-")}</b><small>Part Number: ${esc(partNumberOf(row,row.fgPartCode))}</small></td><td><b>${esc(row.materialOrComponent || row.partCode || "-")}</b><small>Part Number: ${esc(partNumberOf(row))}</small></td><td>${date(row.targetDeliveryDate)}</td><td>${date(row.requiredDate)}</td><td class="mrps-number"><b>${num(row.requirementQty)}</b></td><td class="mrps-number"><b>${num(row.supplyCoverageQty)}</b></td><td>${badge(row._state.label,row._state.tone)}</td><td><button class="mrps-row-action" type="button" data-mrps-detail="${esc(row._id)}">Rincian</button></td></tr>`).join("");
  }
