(async () => {
  ...
  const filteredRows=()=>{const q=searchControl.value.trim().toLowerCase(),phases=new Map(result.groups.map(g=>[g.id,g])),view=viewControl.value;return result.rows.filter(n=>{const g=phases.get(n.groupId);return g.active!==false&&includesGroup(n.groupId)&&`${g.partCode} ${g.partName} ${g.customer}`.toLowerCase().includes(q)&&(view==='delivery'||view==='material'&&n.kind==='material'||view==='resource'&&['process','vendor'].includes(n.kind))&&(kind==="all"||(kind==="production"?["process","vendor","fg"].includes(n.kind):n.kind==="material"));});};
  function render(fitRange=false){
    ...
    $("groups").innerHTML=fgs.filter(g=>`${g.partCode} ${g.partName} ${g.customer}`.toLowerCase().includes(q)).map(g=>`<button type="button" class="ppsb-group" data-group="${esc(g.id)}" aria-pressed="${selected===g.id}"><strong>${esc(g.partCode)}</strong><small>${esc(g.customer)} · ${esc(g.partName)}<br>${num(g.qty)} pcs produksi</small><div><span>${g.phases.length} target tanggal</span><span class="ppsb-badge ${g.status}">${states[g.status]}</span></div></button>`).join("")||'<p class="ppsb-empty">FG tidak ditemukan.</p>';
    ...
    renderGantt(fitRange);renderBoard();analysis.update(seed,result,new Set(result.groups.filter(g=>g.active!==false&&includesGroup(g.id)&&`${g.partCode} ${g.partName} ${g.customer}`.toLowerCase().includes(searchControl.value.trim().toLowerCase())).map(g=>g.id)));
  ...
  function renderBoard(){
    ...
    if(board==="material"){
      ...
      head=["Part / proses","Temuan"];
      rows=members.filter(n=>n.issues.length).map(n=>[editLink(n),n.issues.map(i=>esc(i)).join("<br>")]);
      rows.push(...result.issues.map(i=>["Sumber data",esc(i)]));
      rows.push(...result.groups.filter(g=>includesGroup(g.id)&&g.targetSpareDays<0).map(g=>[esc(g.partCode),`Estimasi tercepat ${esc(time(g.earliestFinish))}; melewati target FG ${num(-g.targetSpareDays)} hari.`]));
    }
