(function () {
  ...
  const table = new DataTable("#bom-table", {
    ...
    columns: [
      ...
      { data: null, render: (_value, _type, row) => `<span class="bom-parent-item">${esc(row.part?.partCode || "—")} <small>${row.part?.partName ? `(${esc(row.part.partName)})` : ""}</small></span>` },
      { data: null, render: (_value, _type, row) => `<span class="bom-muted">${esc(row.part?.partNumber || "—")}</span>` },
