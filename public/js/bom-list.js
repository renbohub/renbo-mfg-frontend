(function () {
  ...
  const table = new DataTable("#bom-table", {
    ...
    columns: [
      ...
      { data: null, render: (_value, _type, row) => `<span class="bom-parent-item">${esc(row.part?.partCode || row.part?.partNumber || "—")} <small>${row.part?.partName ? `(${esc(row.part.partName)})` : ""}</small></span>` },
      ...
      { data: "revision", render: (value) => `<span class="bom-muted">Rev.${String(value ?? 1).padStart(2, "0")}</span>` },
      { data: null, render: (_value, _type, row) => `<b>${detailsOf(row).length} Items</b>` },
      { data: "effectiveDate", render: (value) => `<span class="bom-muted">${formatDate(value)}</span>` },
  ...
  async function setView(view) {
    activeView = ["table", "kanban", "gallery"].includes(view) ? view : "table"; localStorage.setItem("bom-list-view", activeView);
    document.querySelectorAll("[data-bom-view]").forEach((button) => button.classList.toggle("active", button.dataset.bomView === activeView));
    tableView.classList.toggle("d-none", activeView !== "table"); kanbanView.classList.toggle("d-none", activeView !== "kanban"); galleryView.classList.toggle("d-none", activeView !== "gallery");
  ...
  document.querySelectorAll("[data-bom-view]").forEach((button) => button.addEventListener("click", () => setView(button.dataset.bomView)));
