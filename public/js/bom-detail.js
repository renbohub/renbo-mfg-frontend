(function () {
  ...
  function renderDetail(record) {
    ...
    field("notes", record.notes || `Bill of Materials untuk ${partLabel(record.part)}.`); field("componentCount", `${details.length} Item${details.length === 1 ? "" : "s"}`); field("updatedAt", date(record.updatedAt, true));
    document.getElementById("bom-detail-rows").innerHTML = details.length ? details.map((item) => {
      ...
      const materialText = part.itemType === "RAW" && part.rawType === "MATERIAL" ? `${part.material?.materialCode || "Material belum link"} · ${item.materialThickness || 0}×${item.materialWidth || 0}×${item.materialPitch || 0} / CAV ${item.materialCavity || 1} · ${Number(item.grossWeight || 0).toFixed(6)} kg/pcs` : "—";
      return `<tr><td>${Number(item.levelComponent || 1)}</td><td><b class="bom-code-text">${esc(part.partCode || part.partNumber || "—")}</b></td><td>${esc(part.partName || "—")}</td><td class="text-end">${Number(item.qty || 0).toLocaleString("id-ID")}</td><td>${esc(item.uomCode || item.uom?.uomCode || "—")}</td><td>${esc(item.parentDetail?.part?.partName || item.notes || "—")}</td><td>${esc(firstProcess ? processOccurrence(firstProcess) : "—")}</td><td>${esc(materialText)}</td><td class="text-danger">${Number(item.scrapFactor || 0).toLocaleString("id-ID")}%</td><td>${Number(item.leadTime || 0)} ${durationUnitLabel(item.leadTimeUnit)}</td></tr>`;
  ...
  function renderProcesses(record) {
    ...
    const run = processes.reduce((sum, item) => sum + Number(item.cycleTime || 0), 0); document.getElementById("bom-time-caption").textContent = `Total Waktu Produksi (Run): ${run.toLocaleString("id-ID")} Detik`;
