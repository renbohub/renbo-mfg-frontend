  ...
  function create({element,E,onPath}){
    ...
    function render(){
      ...
      if(tab==='capacity'||tab==='bottleneck'){
        ...
        const impact=table(['FG / Need','Delay¹','Qty (pcs)','Review'],impacted.map(g=>{const alternatives=tasks.find(n=>n.groupId===g.id&&n.machineOptions?.length>1),fg=result.rows.find(n=>n.groupId===g.id&&n.kind==='fg');return [escape(g.partCode)+`<small>${date(g.targetDate)}</small>`,`<span class="${g.targetSpareDays<0?'ppsb-delay':''}">${g.targetSpareDays==null?'—':num(Math.max(0,-g.targetSpareDays),2)}</span>`,num(g.qty,0),edit(alternatives?.id||fg?.id,alternatives?'Mesin alternatif':'FG need')];}));
        ...
        panel.innerHTML=table(['Material / FG need','Bruto','Stok terpakai','Kiriman firm','Rencana','Qty net','Beli / minta','Kebutuhan proses','Siap estimasi','Readiness'],scoped.filter(n=>n.kind==='material').sort((a,b)=>(a.requiredAt??a.planned.end??Infinity)-(b.requiredAt??b.planned.end??Infinity)).map(n=>{
        ...
        panel.innerHTML=table(['FG / Need','Rentang jalur','Aktivitas kritis BOM','Spare target','Gantt'],model.paths.filter(p=>visible(p.group)).map(p=>[`${escape(p.group.partCode)}<small>${date(p.group.targetDate)} · ${num(p.group.qty,0)} pcs</small>`,p.unresolved?'Belum terjadwal':`${num(p.length/60)} jam kalender`,p.unresolved?'Selesaikan aktivitas yang belum terjadwal':p.nodes.map(n=>edit(n.id,n.title)).join(' · '),`${num(p.group.targetSpareDays,2)} hari`,p.unresolved?'—':`<button type="button" class="ppsb-analysis-link" data-path="${escape(p.group.id)}">Lihat jalur</button>`]));caption.textContent='CPM dependensi BOM dengan durasi rentang kalender skenario. Cabang paralel dapat sama-sama kritis; ini bukan slack kapasitas mesin. Lihat jalur memberi garis merah pada aktivitas terkait.';
      }else{
        const rows=scoped.flatMap(n=>(n.issues||[]).map(issue=>[edit(n.id,n.partCode),escape(n.title),escape(issue)]));
        rows.push(...(result.issues||[]).map(issue=>['Sumber data','—',escape(issue)]));
        rows.push(...result.groups.filter(g=>visible(g)&&g.targetSpareDays<0).map(g=>[edit(result.rows.find(n=>n.kind==='fg'&&n.groupId===g.id)?.id,g.partCode),'FG need '+date(g.targetDate),`Estimasi selesai melewati target ${num(-g.targetSpareDays,2)} hari.`]));
        panel.innerHTML=table(['Part / FG','Aktivitas','Temuan'],rows);caption.textContent=`${rows.length} temuan untuk FG yang ditampilkan. Klik part untuk membuka adjustment; masalah master perlu diselesaikan pada sumber data.`;
      }
    }
