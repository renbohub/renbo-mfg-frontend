(function () {
  ...
  function renderRiskPreview(payload = {}) {
    ...
    const solver = payload.solver || payload.constraintDetails?.solver || {}; const backward = solver.backward || {}; const forward = solver.forward || {};
    const milestones = (backward.tasks || []).map((task) => `<li><b>${esc(task.id)}</b><span>${dateTime(task.startDate)} → ${dateTime(task.endDate)}</span></li>`).join("");
    root.innerHTML = `<div><small>FG paling awal</small><b>${dateTime(payload.earliestFeasibleFgDate)}</b></div><div><small>Durasi produksi</small><b>${num(payload.exactProductionLeadTimeHours ?? calculation.exactProductionLeadTimeHours)} jam</b></div><div><small>Jam capacity / hari</small><b>${num(calculation.productionHoursPerDay)} jam</b></div><div><small>Delivery paling awal</small><b>${dateTime(payload.earliestFeasibleDeliveryDate)}</b></div><div><small>Hasil</small>${statusBadge(payload.status)}</div><div><small>Constraint</small><b>${esc(friendlyStatus(payload.criticalConstraint))}</b></div>${solver.engine ? `<p><strong>${esc(solver.engine)} ${esc(solver.engineVersion || "")}</strong> · backward ${esc(backward.status || "-")} · forward ${esc(forward.status || "-")} · ${num((backward.tasks || []).length + (forward.tasks || []).length)} task</p>${milestones ? `<details><summary>Milestone netting time CP-SAT</summary><ul>${milestones}</ul></details>` : ""}` : ""}${waived.length ? `<p><strong>Perlu approval risiko:</strong> ${esc(waived.map((row) => row.label).join(", "))}. Preview memakai asumsi Review ini; nilai master dan risiko operasional tetap dipertahankan untuk audit.</p>` : ""}`;
  ...
  function renderRecoveryPlan(payload) {
    ...
    $("demand-drawer-body").innerHTML = `<div class="recovery-overview ${Number(recommendation.recoveryGapDays || plan?.recoveryGapDays) > 0 ? "has-gap" : "is-protected"}"><div><small>CUSTOMER DUE</small><b>${date(recommendation.requestedDeliveryDate || plan?.requestedDeliveryDate)}</b></div><div><small>EARLIEST DELIVERY</small><b>${date(recommendation.earliestFeasibleDeliveryDate || plan?.earliestFeasibleDelivery)}</b></div><div><small>GAP</small><b>${num(recommendation.recoveryGapDays ?? plan?.recoveryGapDays)} hari</b></div><div><small>STATUS</small>${badge(status)}</div></div>
      ...
      ${plan?.approvedBy ? `<div class="recovery-approval-record"><b>${plan.isApplicableToCurrentCalculation === false ? "Approval historis" : "Approved PPIC"} oleh ${esc(plan.approvedBy)}</b><span>${date(plan.approvedAt)} · ${esc(plan.approvalReason || "")}</span></div>` : ""}
