(function () {
  ...
  const builtInScenarios = {
    normal: { name: "Normal", shifts: "1", hours: "8", overtime: "0", saturday: "false", sunday: "false", efficiency: "85" },
  ...
  function resetPresetForm() {
    $("capacity-preset-form").reset(); $("capacity-preset-id").value = ""; $("capacity-preset-month").value = $("capacity-month").value; $("capacity-preset-efficiency").value = "85"; $("capacity-preset-shift-count").value = "1";
  ...
  function renderPresetLibrary() {
    ...
    $("capacity-preset-list").innerHTML = simulationPresets.map((preset) => { const locked = Object.keys(preset.dailyOverrides || {}).filter(isPastDate).length; return `<article class="${preset.id === selectedPresetKey() ? "active" : ""}"><div><b>${esc(preset.name)}</b><small>${esc(preset.month)} · ${num(preset.shiftCount)} shift · ${num(preset.efficiency)}% · ${Object.keys(preset.dailyOverrides || {}).length} override${locked ? ` · ${locked} hari terkunci` : ""}</small></div><div><button class="btn btn-sm btn-outline-secondary" type="button" data-edit-preset="${esc(preset.id)}">Edit</button><button class="btn btn-sm btn-primary" type="button" data-use-preset="${esc(preset.id)}">Gunakan</button></div></article>`; }).join("") || '<div class="capacity-empty">Belum ada preset pada bulan ini.</div>';
  }
  function syncScenarioOptions(preferredId = selectedPresetKey()) {
    if (capacityPlanningMode === "SIMULATION") {
      ...
      const presetOptions = simulationPresets.length ? `<optgroup label="Preset dari Simulation">${simulationPresets.map((preset) => `<option value="${esc(preset.id)}">${esc(preset.name)}</option>`).join("")}</optgroup>` : "";
      $("capacity-scenario").innerHTML = `<optgroup label="Setup standar"><option value="normal">Normal</option><option value="max">Maximum</option><option value="custom">Custom Aktif</option></optgroup>${presetOptions}`;
      $("capacity-scenario").value = simulationPresets.some((preset) => preset.id === preferredId) || builtInScenarios[preferredId] ? preferredId : "normal";
    }
    renderPresetLibrary();
  }
  async function loadSimulationPresets(preferredId = selectedPresetKey()) {
    const month = $("capacity-month").value; if (!month) return;
    const payload = await api(`/modules/api/planning-ppic/capacity-planning/presets?month=${encodeURIComponent(month)}`); simulationPresets = payload.presets || []; syncScenarioOptions(preferredId); const selected = activePreset(); if (selected) fillPresetForm(selected); else resetPresetForm();
  }
  function updateCapacityModeUi() {
    ...
    $("capacity-recommend").classList.toggle("d-none", !selectedPlan || (simulation && !activeScenarioKey()) || (!simulation && !["Draft", "Confirmed"].includes(selectedPlan.status) && !(["Released", "In Progress"].includes(selectedPlan.status) && selectedPlan.replanRequired)));
    ...
    if (title) title.textContent = simulation ? "Capacity Simulation Workspace" : "Production Capacity Workspace";
    const help = $("capacity-main-view-help");
    if (help) help.textContent = simulation
      ? activePreset() ? `Preset ${activePreset().name}. Langkah berikutnya: Auto Allocation, periksa hasilnya, lalu klik Tetapkan Preset sebagai Production. Hari lampau terkunci.` : "Pilih MPP dan preset bulanan, lalu jalankan Auto Allocation."
      : selectedPlan?.replanRequired ? `Replan wajib: ${selectedPlan.replanReason || "target delivery berubah"}` : activePreset() ? `Production memakai parameter preset ${activePreset().name}. Allocation tetap tersimpan sebagai Production, bukan Simulation.` : "Sumber resmi untuk generate dan revisi Draft Daily Production Plan (DPP).";
    ...
    $("capacity-production-guide-steps").textContent = simulation && selectedPlan && !canAdoptPlan
      ...
      : "Simulation → pilih MPP dan preset → Auto Allocation → Tetapkan Preset sebagai Production.";
  }
  ...
  function renderGantt() {
    ...
    const body = rows.slice(0, 150).map((row) => `<div class="capacity-gantt-label"><b>${esc(row.partCode)} · ${esc(row.processCode)}</b><span>${esc(row.reference)}${row.lineNumber ? ` · L${num(row.lineNumber)}` : ""}</span><small>${esc(row.machine.machineCode)} · ${esc(row.machine.machineName || "-")}</small></div>${snapshot.dates.map((date) => {
      ...
      return `<div class="capacity-gantt-slot"><button type="button" class="capacity-gantt-bar ${source} ${esc(entry.status)}" data-machine="${esc(row.machine.id)}" data-date="${esc(date)}" title="${esc(row.reference)} · ${qty(entry.qty, entry.uomCode)} ${esc(entry.uomCode || "")} · ${num(entry.minutes, 1)} menit"><b>${qty(entry.qty, entry.uomCode)}</b><span>${num(entry.minutes)}m</span></button></div>`;
    }).join("")}`).join("");
  ...
  function renderDeliveryCoverage() {
    ...
    $("capacity-delivery-coverage-body").innerHTML = (coverage.phases || []).map((phase) => `<tr><td><b>${phase.phaseNumber ? `Phase ${num(phase.phaseNumber)}` : "Belum dibuat"}</b></td><td>${esc(phase.planNumber)}</td><td><b>${esc(phase.partCode)}</b></td><td>${esc(phase.targetType)} · ${esc(phase.targetCode)}</td><td>${esc(phase.plannedDate)}</td><td class="ppic-number">${qty(phase.cumulativeRequiredQty, phase.uomCode)} ${esc(phase.uomCode || "")}</td><td class="ppic-number">${qty(phase.plannedQtyByDueDate, phase.uomCode)}</td><td class="ppic-number">${qty(phase.shortageQty, phase.uomCode)}</td><td><span class="capacity-readiness-badge ${phase.status === "COVERED" ? "ready" : "blocked"}">${esc(phase.status)}</span></td></tr>`).join("") || '<tr><td colspan="9" class="capacity-empty">Belum ada delivery phase pada MPS sumber.</td></tr>';
  }
  function renderVendorAllocations() {
    ...
    $("capacity-vendor-allocation-body").innerHTML = (snapshot?.vendorAssignments || []).map((item) => {
      ...
      const action = ["MANUAL", "RECOMMENDED"].includes(item.source) && item.allocationId && !isPastDate(item.sendDate || item.scheduleDate)
        ? `<div class="d-flex gap-1"><button class="btn btn-sm btn-outline-primary" type="button" data-edit-manual-allocation="${esc(item.allocationId)}">Edit</button><button class="btn btn-sm btn-outline-danger" type="button" data-remove-manual-allocation="${esc(item.allocationId)}" data-plan="${esc(item.planNumber)}">Hapus</button></div>`
        : "-";
    ...
    const manualAssignment = (item) => {
      ...
      if (["MANUAL", "RECOMMENDED"].includes(item.source) && item.allocationId) {
        const schedule = item.plannedStartTime && item.plannedEndTime
          ? `<small class="d-block">${esc(item.plannedStartTime)}–${esc(item.plannedEndTime)} · ${esc(item.capacityMode || "NORMAL")}${item.deliveryPhaseNumber ? ` · Delivery Phase ${num(item.deliveryPhaseNumber)}` : ""}${item.transferBatchNumber ? ` · Transfer Batch ${num(item.transferBatchNumber)}` : ""}</small>`
          : "";
        return `<div class="d-flex flex-column gap-1"><small class="text-primary fw-semibold">${item.source === "RECOMMENDED" ? "Rekomendasi sistem" : "Draft allocation MPP"}</small>${schedule}<div class="d-flex gap-1"><button class="btn btn-sm btn-outline-primary" type="button" data-edit-manual-allocation="${esc(item.allocationId)}">Edit</button><button class="btn btn-sm btn-outline-danger" type="button" data-remove-manual-allocation="${esc(item.allocationId)}" data-plan="${esc(item.planNumber || item.reference)}">Hapus</button></div></div>`;
      }
  ...
  function issueActions(issue) {
    ...
    if (issue.allocationId) {
      const label = issue.code === "PLAN_VENDOR_REQUIRED" ? "Pilih vendor" : /PREDECESSOR|SEQUENCE|OVERLAP/.test(issue.code) ? "Ubah successor" : "Ubah allocation";
      actions.push(`<button type="button" data-edit-manual-allocation="${esc(issue.allocationId)}">${label} →</button>`);
    }
    if (issue.relatedAllocationId) actions.push(`<button type="button" data-edit-manual-allocation="${esc(issue.relatedAllocationId)}">Ubah predecessor →</button>`);
  ...
  function renderReadiness() {
    ...
    const selectedPlan = plans.find((plan) => plan.planNumber === $("capacity-plan").value);
    const canRecommendProduction = selectedPlan && (["Draft", "Confirmed"].includes(selectedPlan.status) || (["Released", "In Progress"].includes(selectedPlan.status) && selectedPlan.replanRequired));
    ...
    $("capacity-readiness-list").innerHTML = ranked.slice(0, 80).map((issue) => `<div class="capacity-issue ${esc(issue.severity)}"><i></i><div><small>${esc(issue.category || "CAPACITY")} · ${issue.canOverride ? "Perlu approval override" : "Wajib diperbaiki"}</small><b>${esc(issue.code)}</b><span>${esc(issue.message)}</span>${issue.resolution ? `<em class="d-block mt-1">Tindakan: ${esc(issue.resolution)}</em>` : ""}${issueActions(issue)}</div></div>`).join("") || '<div class="capacity-empty">Routing, urutan proses, vendor, mesin, qty, dan delivery phase siap digunakan.</div>';
  }
  ...
  async function runCapacityRecommendation(button, requirePreset = false) {
    ...
    try {
      ...
      const message = result.ready
        ? `${requirePreset ? `Preset ${preset.name} diterapkan. ` : ""}${num(result.allocationCount)} slot rekomendasi dibuat dan blocker capacity sudah terselesaikan.`
        : `${requirePreset ? `Preset ${preset.name} diterapkan, tetapi ` : ""}${num(result.allocationCount)} slot dibuat; ${num(result.blockers?.length)} blocker masih perlu ditangani.`;
      alert(message, result.ready ? "success" : "warning");
      await load();
    } catch (error) { alert(error.message); }
    finally { button.disabled = false; }
  }
  $("capacity-recommend").addEventListener("click", () => runCapacityRecommendation($("capacity-recommend")));
  $("capacity-use-preset").addEventListener("click", () => runCapacityRecommendation($("capacity-use-preset"), true));
  ...
  function setupCapacityDialogs() {
    ...
    [$("capacity-allocation-dialog"), scenarioDialog].filter(Boolean).forEach((dialog) => {
    ...
    $("capacity-adopt-simulation")?.addEventListener("click", async () => {
      ...
      if (!confirm(`Tetapkan preset ${activePreset()?.name || scenarioKey} sebagai Production Capacity? Hanya hari ini dan hari mendatang yang akan diganti; hari yang sudah lewat tetap terkunci.`)) return;
      try {
