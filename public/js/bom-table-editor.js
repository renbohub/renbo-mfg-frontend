(function () {
  ...
  function materialConsumptionSummary(row) {
    ...
    return `<button class="bom-table-material-consumption" type="button" data-edit-material><b>${escapeHtml(detail)}</b><small>Quotation: ${window.BomMeasurements.materialSummary(row, true)}</small><small>Actual: ${window.BomMeasurements.materialSummary(row)}</small><small>${activeScheme} · Cavity ${Number(activeCavity || 1)} · Gross ${Number(row.grossWeight || 0).toFixed(2)} kg/pcs</small><em>Edit konsumsi</em></button>`;
  }
  ...
  function updateRow(event) {
    ...
    if (field === "partId") {
      ...
      for (const key of [...window.BomMeasurements.materialFields, "materialThickness", "materialWidth", "materialPitch", "materialCavity", "materialDensity", "materialFormId", "alternateMaterialFormId", "alternateMaterialPitch", "alternateMaterialCavity", "defaultGrossWeight", "alternateGrossWeight", "grossWeight"]) row[key] = null;
      row.materialScheme = "DEFAULT";
  ...
  function openMaterialDialog(row) {
    ...
    document.getElementById("bom-quotation-material").innerHTML = window.BomMeasurements.materialInputs(row, "bom-material-dialog-grid");
  ...
  ["bom-material-thickness", "bom-material-width", "bom-material-pitch", "bom-material-cavity"].forEach((id) => document.getElementById(id).addEventListener("input", () => { materialDialogGross(false); materialDialogGross(true); }));
  ...
  document.getElementById("bom-material-save").addEventListener("click", () => { const row = editingMaterialRow(); const pitchInput = document.getElementById("bom-material-pitch"); const cavityInput = document.getElementById("bom-material-cavity"); const scheme = document.getElementById("bom-material-scheme").value; const defaultFormId = document.getElementById("bom-material-form").value || null; const altFormId = document.getElementById("bom-material-alt-form").value || null; const altPitch = Number(document.getElementById("bom-material-alt-pitch").value) || null; const altCavity = Math.max(1, Math.round(Number(document.getElementById("bom-material-alt-cavity").value || 1))); if (!row || !Array.from(materialDialog.querySelectorAll("input")).every((input) => input.reportValidity())) return; if (!defaultFormId) return alert("Pilih Material Form default."); if (altFormId && altFormId === defaultFormId) return alert("Material Form alternatif harus berbeda dari default."); if (scheme === "ALTERNATIVE" && (!altFormId || !altPitch)) return alert("Lengkapi Material Form dan Pitch pada skema alternatif."); materialDialog.querySelectorAll("[data-quotation-material]").forEach((input) => { row[input.dataset.quotationMaterial] = window.BomMeasurements.parse(input.value); }); row.materialThickness = Number(document.getElementById("bom-material-thickness").value) || null; row.materialWidth = Number(document.getElementById("bom-material-width").value) || null; row.materialPitch = Number(pitchInput.value) || null; row.materialCavity = Math.max(1, Math.round(Number(cavityInput.value || 1))); row.materialDensity = Number(document.getElementById("bom-material-density").value) || null; row.materialFormId = defaultFormId; row.materialScheme = scheme; row.defaultGrossWeight = materialDialogGross(false); row.alternateMaterialFormId = altFormId; row.alternateMaterialPitch = altPitch; row.alternateMaterialCavity = altFormId ? altCavity : null; row.alternateGrossWeight = altFormId && altPitch ? materialDialogGross(true) : null; row.grossWeight = scheme === "ALTERNATIVE" ? Number(row.alternateGrossWeight || 0) : row.defaultGrossWeight; materialDialog.close(); renderRows(); });
